...
func W文本_到大写(要转换的文本 string) string {
	return strings.ToUpper(要转换的文本)
}
func W文本_到小写(要转换的文本 string) string {
...
func W文本_取右边(被查找的文本 string, 欲寻找的文本 string, 起始寻找位置 int, 是否不区分大小写 bool) string {
	起始寻找位置 = func() int {
		if 起始寻找位置 == 0 {
			return len(被查找的文本) + 1
		}
...
func W文本_取左边(被查找的文本 string, 欲寻找的文本 string, 起始寻找位置 int, 是否不区分大小写 bool) string {
	if 起始寻找位置 <= 0 {
		起始寻找位置 = 1
	}

	找到的位置 := strings.Index(被查找的文本, 欲寻找的文本)
	if 是否不区分大小写 {
		找到的位置 = strings.Index(strings.ToLower(被查找的文本), strings.ToLower(欲寻找的文本))
	}
...
// 取一段文本的长度，中文算两个长度，字母算一个长度
func W文本_取长度(源文本 string) int {
	return utf8.RuneCountInString(源文本)
}

func W文本_删首尾空(参_内容 string) string {
	if 参_内容 == "" {
		return ""
	}
	返回值 := strings.TrimSpace(参_内容)
	return 返回值
}
