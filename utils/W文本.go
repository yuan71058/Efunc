...
func W文本_倒取出中间文本(欲取全文本 string, 右边文本 string, 左边文本 string, 倒数搜寻位置 int, 是否不区分大小写 bool) string {
	倒数搜寻位置 = F_选择(倒数搜寻位置 <= 0, -1, 倒数搜寻位置).(int)
...
func W文本_取随机IP() string {
	...
	switch x {
	case 1:
		return IP_10进制转IP(F_取随机数(607649792, 608174079))
	case 2:
		return IP_10进制转IP(F_取随机数(1038614528, 1039007743))
	case 3:
		return IP_10进制转IP(F_取随机数(1783627776, 1784676351))
	case 4:
		return IP_10进制转IP(F_取随机数(2035023872, 1039007743))
	case 5:
		return IP_10进制转IP(F_取随机数(2078801920, 2079064063))
	case 6:
		return IP_10进制转IP(-1 * F_取随机数(1948778497, 1950089216))
	case 7:
		return IP_10进制转IP(-1 * F_取随机数(1425014785, 1425539072))
	case 8:
		return IP_10进制转IP(-1 * F_取随机数(1235419137, 1236271104))
	case 9:
		return IP_10进制转IP(-1 * F_取随机数(768606209, 770113536))
	case 10:
		return IP_10进制转IP(-1 * F_取随机数(564133889, 569376768))
	}
...
func W文本_去重复文本(原文本 string, 分割符 string) string {
	...
	for _, 成员 := range 局_数组 {
		if 内部_数组成员是否存在_文本(局_数组1, 成员) == -1 {
			局_数组1 = F_加入成员(局_数组1, 成员)
			局_文本 += 成员 + 分割符
		}
	}

	局_文本 = F_取文本左边(局_文本, F_取文本长度(局_文本)-F_取文本长度(分割符))
	return 局_文本
}
