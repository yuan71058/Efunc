...
// 返回:
//   - int: 加入后队列中的元素数量
func (q *QueueGeneric[T]) Enqueue(v T) int {
	q.l.Lock()
	defer q.l.Unlock()
	q.data.PushFront(v)
	return q.data.Len()
...
func (q *QueueGeneric[T]) Dequeue() (T, bool) {
	q.l.Lock()
	defer q.l.Unlock()
	iter := q.data.Back()
	if iter == nil {
