// Package utils 提供英文命名的通用工具函数库，涵盖类型转换、编码处理、文本操作、文件管理、
// 网络请求、加密校验、并发安全等常用功能，是 Go 语言版的精易模块（英文版）。
package utils
...
// ToFloat64 将任意类型的值转换为 float64 类型。
...
// ToStruct 将通用类型（如 Map、JSON 字符串）转换为指定的结构体。
// 内部使用 gconv.Struct，支持 map、JSON 字符串等作为输入源。
// 结构体字段可通过 tag（如 json/c/gconv）指定映射名称。
//
// 参数:
//   - source: 源数据，支持 map、JSON 字符串等
//   - target: 目标结构体的指针，转换结果将写入该结构体
//
// 返回:
//   - error: 转换失败时返回错误信息
func ToStruct(source interface{}, target interface{}) error {
	return gconv.Struct(source, target)
}

// Ternary 泛型三元运算符，根据条件返回两个值中的一个。
...
//
// 参数:
//   - condition: 条件表达式，true 时返回 trueVal，false 时返回 falseVal
//   - trueVal: 条件为真时的返回值
//   - falseVal: 条件为假时的返回值
//
// 返回:
...
// MultiSelect 泛型多项选择器，根据索引从数组中选取对应元素。
// 索引从 0 开始，越界时返回默认值。
//
// 类型参数:
//   - T: 任意类型
...
// 示例:
//
//	MultiSelect(1, []string{"a","b","c"}, "x")  // "b"
//	MultiSelect(5, []int{1,2,3}, 0)              // 0（越界返回默认值）
func MultiSelect[T any](index int, arr []T, defaultVal T) T {
	if len(arr) < index+1 {
		return defaultVal
	}
	return arr[index]
...
// FormatText 使用格式化字符串生成文本，等同于 fmt.Sprintf。
...
// 示例:
//
//	FormatText("姓名:%s,年龄:%d", "张三", 25)  // "姓名:张三,年龄:25"
func FormatText(format string, args ...interface{}) string {
	return fmt.Sprintf(format, args...)
}

// FormatJSON 将 JSON 字符串进行缩进格式化，便于阅读。
...
// ToStringArray 将通用型变量（interface{}）转换为字符串数组。
// 支持数组、切片类型的元素逐一转换；非数组类型作为单元素数组返回。
// 使用反射（reflect）实现类型判断和元素提取。
//
// 参数:
//   - value: 待转换的值，支持 []string、[]int 等数组/切片类型
//
// 返回:
//   - []string: 转换后的字符串数组
func ToStringArray(value interface{}) []string {
	var result []string
	aa := reflect.TypeOf(value).Kind()
	if aa == reflect.Array || aa == reflect.Slice {
		for v := range reflect.ValueOf(value).Len() {
			index := reflect.ValueOf(value).Index(v)
...
// RandomInt 生成指定范围内 [min, max] 的随机整数（线程安全）。
//
// 参数:
//   - min: 最小值（包含）
//   - max: 最大值（包含）
